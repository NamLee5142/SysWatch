...
int main() {
    ...
    std::thread reader([port] {
        ...
        assert(ok);
        assert(response.find("204 No Content") != std::string::npos || response.find("200 OK") != std::string::npos);
    });

    std::thread writer([&agent] {
        agent.start();
    ...
    std::cout << "HTTPServer concurrency test passed." << std::endl;
    return 0;
}
